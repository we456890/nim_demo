...
public class PagerSlidingTabStrip extends HorizontalScrollView implements OnPageChangeListener {
    private int[] imgInitializeRes={
            R.mipmap.menu_work_unselected,
            R.mipmap.menu_info_unselected,
            R.mipmap.menu_contacts_unselected,
            R.mipmap.menu_menu_unselected};
    private int[] imgSelectedRes={
            R.mipmap.menu_work_selected,
            R.mipmap.menu_info_selected,
            R.mipmap.menu_contacts_selected,
            R.mipmap.menu_menu_selected
            };
    ...
    private int checkedTextColor = R.color.colortitle;

    private int unCheckedTextColor = R.color.color_grey_999999;
    ...
    public PagerSlidingTabStrip(Context context, AttributeSet attrs, int defStyle) {
        ...
        checkedTextColor = a.getResourceId(R.styleable.TwoTextView_ttLeftTextColor, R.color.colortitle);
        unCheckedTextColor = a.getResourceId(R.styleable.TwoTextView_ttLeftTextColor,R.color.color_grey_999999);
        indicatorHeight = a
    ...
    public void notifyDataSetChanged() {
        ...
        for (int i = 0; i < tabCount; i++) {
            addTabView(i, pager.getAdapter().getPageTitle(i).toString(), imgInitializeRes[i]);
        }
    ...
    private void setChooseTabViewTextColor(int position) {
        ...
        for (int i = 0; i < childCount; ++i) {
            ...
            image = (ImageView) tabView.findViewById(R.id.tab_image_label);
            textView = (TextView) tabView.findViewById(R.id.tab_title_label);
            if (i == position) {
//                imgInitializeRes[i]
                image.setImageResource(imgSelectedRes[i]);
                textView.setTextColor(getResources().getColor(checkedTextColor));
            } else {
                image.setImageResource(imgInitializeRes[i]);
                textView.setTextColor(getResources().getColor(unCheckedTextColor));
            }
        }
    }

    private void addTabView(final int position, String title, int imgRes) {
        View tabView = null;
        ...
        ImageView tab_image_label = (ImageView) tabView.findViewById(R.id.tab_image_label);
        ...
        if (titleTV != null) {
            titleTV.setTextSize(TypedValue.COMPLEX_UNIT_PX, needAdaptation ? resources.getDimensionPixelSize(R.dimen.text_size_8) : resources.getDimensionPixelSize(R.dimen.text_size_10));
            titleTV.setText(title);
            tab_image_label.setImageResource(imgRes);
        }
